function [ishappy, epslevel, cutoff] = standardCheck(f, values, vscl, hscl, pref)
...
%   allows additional preferences to be passed. VALUES is a matrix of the
%   function values of F at the corresponding interpolation points. VSCL is an
...
% Scale TOL by the MAX(HSCL, VSCL/||F||).
...
% HSCL >> 1. For functions on a single domain with no breaks, this scaling has
% no effect since HSCL = 1. 
nrmf = max(abs(values), [], 1);
if ( nargin < 3 || isempty(vscl) )
    vscl = nrmf;
end
tol = tol.*max(hscl, vscl./nrmf);
